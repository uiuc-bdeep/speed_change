...
agg_Q <- function(my.model,  model.number, my.data, my.weights){
 ...
 bb$rm <- as.numeric(substr(row.names(bb),4,5))
 ...
 cc <- as.data.frame(cc[19:108,19:108])
 cc$c_o <- as.numeric(substr(row.names(cc),2,2))
 cc$rm_o <- as.numeric(substr(row.names(cc),4,5))
 ccl <-list()
 ...
 for(i in 1:90){#i<-1
  ...
  rm_d<-as.numeric(substr(colnames(cct)[1],4,5))
 ...
 ccb$rq_o <- floor(ccb$rm_o/3)
 ccb$rq_d <- floor(ccb$rm_d/3)
...
# calculate quarterly coefficients by model
agg_Qm <- function(my.model,  model.number, my.data, my.weights){
 #my.model <- m0c
 #model.number <- 0
 #my.data <- ES
 #my.weights <- WW
