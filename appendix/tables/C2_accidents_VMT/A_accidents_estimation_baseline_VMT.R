...
# output
OP.p <-"appendix/tables/C2_accidents_VMT/A_accidents_estimation_baseline_VMT.csv"
...
# read VMT
VM <- readRDS("data/roads/vmt/segments_vmt.rds")
VM$dVMT <- (VM$VMT2 - VM$VMT1)/VM$VMT1
VM$dVMT[VM$dVMT > 2] <- 2
VM$dVMT[VM$dVMT < -0.5] <- -0.5
PD <- merge(PD,VM, by="point_id")
PD$VMT <- ifelse(PD$date_m < as.Date("2016-01-01"), 0,PD$dVMT)
...
f0c <- as.formula(paste("accidents_m ~ ",
                       ...
                       "camera + camera_D + t + log_fuel + log_cameras + VMT  | point_id"))
...
m1 <- femlm(accidents_m ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                     camera+camera_D+VMT | point_id + mon,
           data = C1)
summary(m1)
