...
library(optmatch)
library(stringr)
source("appendix/tables/B3_robustness/reg_functions.R")
...
# output
OP.p <-"appendix/tables/B3_robustness/C_linear.csv"
...
# mean accidents per month in the pre
PD <- PD[PD$acc_pre_seg >=1,]
PD0 <- PD[PD$pre %in% 1,]
PD0$acc_mean_pre <- ave(PD0$accidents_m, PD0$point_id, FUN=mean)
PD0 <- PD0[!duplicated(PD0$point_id),c("point_id", "acc_mean_pre")]
PD <- merge(PD, PD0, by="point_id", all.x=T)
PD$acc_r <- PD$accidents_m/PD$acc_mean_pre
...
# Event Study
m0 <- felm(acc_r ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                          camera + camera_D + t + log_fuel + log_cameras  | point_id,
            data = ES)

# Event Study CATT
m0c <- felm(f0c,data = ES)


# With all controls
m1 <- felm(acc_r ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                     camera + camera_D | point_id + mon,
           data = C1)

# With controls located more than 1.6km of treatment
m2 <- felm(acc_r ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                          camera + camera_D  | point_id + mon,
           data = C2)

# With matched controls located more than 1.6km of treatment
m3 <- felm(acc_r ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                          camera + camera_D  | point_id + mon,
           data = C3)
...
a0c <- as.data.frame(summary(m0c)$coefficients)
a0c <- a0c[1:108,]
a0c$c <- str_split_fixed(row.names(a0c), "m", 2)[,1]
a0c$c <- as.numeric(gsub("c","",a0c$c))
a0c$m <- as.numeric(str_split_fixed(row.names(a0c), "m", 2)[,2])

a0c$rn <- row.names(a0c)
a0c <- merge(a0c,WW, by="c", all.x = T)
row.names(a0c) <- a0c$rn

a0c$q <- floor(a0c$m/3)
a0c$qc <- paste(a0c$q, a0c$c)

a0c$bw <- a0c$Estimate*a0c$w
a0c$b <- ave(a0c$bw, a0c$q, FUN=sum)
aa <- a0c[!duplicated(a0c$q),]
sum(a0c$w[a0c$q == 5])
