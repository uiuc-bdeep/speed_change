...
# output
OP.p <-"appendix/tables/B3_robustness/D_negbin.csv"
...
m0 <- femlm(accidents_m ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                          camera + camera_D + t + log_fuel + log_cameras  | point_id,
            data = ES, family = "gaussian")


# Event Study CATT
m0c <- femlm(f0c,data = ES, family = "gaussian")


# With all controls
m1 <- femlm(accidents_m ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                     camera + camera_D | point_id + mon,
           data = C1, family = "gaussian")

# With controls located more than 1.6km of treatment
m2 <- femlm(accidents_m ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                          camera + camera_D  | point_id + mon,
           data = C2, family = "gaussian")

# With matched controls located more than 1.6km of treatment
m3 <- femlm(accidents_m ~ Q0 + Q1 + Q2 + Q3 + Q4 + Q5 +
                          camera + camera_D  | point_id + mon,
           data = C3, family = "gaussian")


# Combine all results --------------------------------------------

# get outputs in relative terms and clustered s.e. 
a0 <- summary(m0, se = "cluster", cluster = ES$StreetName)
a1 <- summary(m1, se = "cluster", cluster = C1$StreetName)
a2 <- summary(m2, se = "cluster", cluster = C2$StreetName)
a3 <- summary(m3, se = "cluster", cluster = C3$StreetName)

a0c <- gentabL(m0c, "m0c", ES)  se = "cluster", cluster = ES$StreetName)
