...
# output
OP.p <-"appendix/tables/C4_CBA_exposure/"
...
RR <- VM[abs(VM$marg_ratio_3 - VM$marg_ratio_2) > 0,]
RR$time_diff_23 <- (RR$dur2 - RR$dur3)*60
RR$marg_gain <- ((RR$dis2*RR$marg_ratio_2)*60)*0.20
summary(RR$marg_gain)
summary(RR$time_diff_23)

RR$int_gain <- RR$time_diff_23 - RR$marg_gain
RX <- RR[RR$int_gain>=0,]

RX$dur2s <- RX$dur2*60
RX$int_gain_r <- RX$int_gain/RX$dur2s

ggplot() +
 geom_histogram(aes(RX$int_gain_r, y=..density..)) +
 geom_vline(aes(xintercept=mean(RX$int_gain_r)), linetype=2) +
 labs(x="relative time change", y = "") +
 scale_x_continuous(breaks = seq(0,240,60))
# recalcul
VM$marg_l2 <- VM$dis2*VM$marg_ratio_2
VM$marg_l3 <- VM$dis3*VM$marg_ratio_3
VM$oth_l2 <- VM$dis2*(1-VM$marg_ratio_2)
VM$oth_l3 <- VM$dis3*(1-VM$marg_ratio_3)

m2 <- lm(dur2~-1+marg_l2+oth_l2, data=VM)
summary(m2)

m3 <- lm(dur3~-1+marg_l3+oth_l3, data=VM)
summary(m3)

VM$expTT_22 <- VM$marg_l2*1.045214 + VM$oth_l2*1.42
VM$expTT_33 <- VM$marg_l3*0.863092 + VM$oth_l3*1.42
VM$expTT_23 <- VM$marg_l2*0.863092 + VM$oth_l2*1.42



VM$dur_23 <- VM$dur2*VM$marg_ratio_2*(70/90) +
             VM$dur2*(1-VM$marg_ratio_2)

VM$rr_save <- VM$dur_23 - VM$dur3
RR <- VM[abs(VM$marg_ratio_3 - VM$marg_ratio_2) > 0,]
summary(RR$rr_save)
...
VM <- VM[,c("ID_ORDEM",
            "marg_ratio_1","ratio.marg_spill_1000_1",
            "ratio.marg_spill_3000_1","ratio.marg_spill_5000_1",
            "marg_ratio_2","ratio.marg_spill_1000_2",
            "ratio.marg_spill_3000_2","ratio.marg_spill_5000_2",
            "marg_ratio_3","ratio.marg_spill_1000_3",
            "ratio.marg_spill_3000_3","ratio.marg_spill_5000_3")]
VM$marg_ratio_2[is.na(VM$marg_ratio_2)] <- 0
VM$ratio.marg_spill_1000_2[is.na(VM$ratio.marg_spill_1000_2)] <- 0
VM$ratio.marg_spill_3000_2[is.na(VM$ratio.marg_spill_3000_2)] <- 0
VM$ratio.marg_spill_5000_2[is.na(VM$ratio.marg_spill_5000_2)] <- 0
VM$marg_ratio_3[is.na(VM$marg_ratio_3)] <- 0
VM$ratio.marg_spill_1000_3[is.na(VM$ratio.marg_spill_1000_3)] <- 0
VM$ratio.marg_spill_3000_3[is.na(VM$ratio.marg_spill_3000_3)] <- 0
VM$ratio.marg_spill_5000_3[is.na(VM$ratio.marg_spill_5000_3)] <- 0


CT <- merge(CT, VM, by="ID_ORDEM", all.x=T)
CT$marg_ratio_1 <- CT$marg.ratio
CT$ratio.marg_spill_1000_1 <- CT$ratio.marg_spill_1000
CT$ratio.marg_spill_3000_1 <- CT$ratio.marg_spill_3000
CT$ratio.marg_spill_5000_1 <- CT$ratio.marg_spill_5000
...
for(i in 1:3){
CT[,"marg.ratio"] <- CT[,paste0("marg_ratio_",i)]
CT[,"ratio.marg_spill_1000"] <- CT[,paste0("ratio.marg_spill_1000_",i)]
CT[,"ratio.marg_spill_3000"] <- CT[,paste0("ratio.marg_spill_3000_",i)]
CT[,"ratio.marg_spill_5000"] <- CT[,paste0("ratio.marg_spill_5000_",i)]
...
write.csv(MC, paste0(OP.p,"cost_scenario_",i, ".csv"), row.names=F)
} 
